<x-publik>
    <x-slot name="title">Hymne KAMMI</x-slot>
    <div class="page-header">
        <div class="page-header-inner">
            <div class="breadcrumb"><a href="/">Beranda</a><span class="breadcrumb-sep">›</span><span>Tentang</span><span class="breadcrumb-sep">›</span><span>Hymne KAMMI</span></div>
            <h1>{{ $konten->judul ?? 'Hymne KAMMI' }}</h1>
        </div>
    </div>
    <div style="padding:56px 2.5rem;max-width:800px;margin:0 auto;">
        ...
        <div class="fade-up" style="background:white;border-radius:12px;padding:20px;border:1px solid #ede8e3;margin-bottom:24px;">
            <p style="font-size:13px;color:#9ca3af;margin-bottom:10px;">Audio Hymne KAMMI</p>
            <audio controls style="width:100%;"><source src="{{ Storage::url($konten->file_audio) }}" type="audio/mpeg"></audio>
        </div>
        @endif
        @if($konten->konten)
        ...
        @else
            <div style="text-align:center;padding:80px 24px;color:#9ca3af;">
                ...
                <p>Konten Hymne KAMMI belum diisi. Tambahkan melalui dashboard admin.</p>
            </div>
        @endif
    </div>
</x-publik>
